...
export const useCreateTokenForm = () => {
  ...
  const selectedAgentUuids = computed(() => {
    ...
    for (const uuid of parseScopeList(otherAgentUuidsText.value))
      dedup.add(uuid);
    return [...dedup];
  });

  const selectedKvNamespaces = computed(() =>
    uniqueList(parseScopeList(kvNamespacesText.value)),
  );
  const selectedKvReadTargets = computed(() =>
    uniqueList(parseTextList(kvReadTargetsText.value)),
  );
  const selectedKvWriteTargets = computed(() =>
    uniqueList(parseTextList(kvWriteTargetsText.value)),
  );
  const selectedKvDeleteTargets = computed(() =>
    uniqueList(parseTextList(kvDeleteTargetsText.value)),
  );
  ...
  const loadKnownAgentUuids = async (options: { force?: boolean } = {}) => {
    ...
    if (!token) {
      ...
      listAgentsError.value =
        "Current backend token is empty. Cannot list agents.";
      return;
    }
    ...
    if (!options.force && cacheKey && cachedAgentUuidsByBackend.has(cacheKey)) {
      ...
      selectedKnownAgentUuids.value = knownAgentUuids.value.filter((uuid) =>
    ...
    const request = (async () => {
      try {
        const result = await wsRpcCall<unknown>(
          url,
        ...
        selectedKnownAgentUuids.value = nextUuids.filter((uuid) =>
          selectedSet.has(uuid),
        );
      } catch (error: any) {
  ...
  const generalPermissions = computed<PermissionEntry[]>(() => {
    ...
    for (const field of staticReads.value)
      permissions.push({ static_monitoring: { read: field } });

    if (dynamicWrite.value) permissions.push({ dynamic_monitoring: "write" });
    for (const field of dynamicReads.value)
      permissions.push({ dynamic_monitoring: { read: field } });

    if (taskListen.value) permissions.push({ task: "listen" });
    for (const taskType of taskCreate.value)
      permissions.push({ task: { create: taskType } });
    for (const taskType of taskRead.value)
      permissions.push({ task: { read: taskType } });
    for (const taskType of taskWrite.value)
      permissions.push({ task: { write: taskType } });
    ...
    if (nodegetListAllAgentUuid.value)
      permissions.push({ nodeget: "list_all_agent_uuid" });
    ...
    if (crontabResultDelete.value)
      permissions.push({ crontab_result: "delete" });
  ...
  const kvPermissions = computed<PermissionEntry[]>(() => {
    ...
    for (const target of selectedKvReadTargets.value)
      permissions.push({ kv: { read: target } });
    for (const target of selectedKvWriteTargets.value)
      permissions.push({ kv: { write: target } });
    for (const target of selectedKvDeleteTargets.value)
      permissions.push({ kv: { delete: target } });

    return permissions;
  });

  const selectedPermissionCount = computed(
    () => generalPermissions.value.length + kvPermissions.value.length,
  );

  const generalScopes = computed<TokenLimitEntry["scopes"]>(() => {
    if (generalScopeMode.value === "global") return ["global"];
    return selectedAgentUuids.value.map((agentUuid) => ({
  ...
  const kvScopes = computed<TokenLimitEntry["scopes"]>(() => {
    ...
    return selectedKvNamespaces.value.map((kvNamespace) => ({
  ...
  const creatorToken = computed(
    () => currentBackend.value?.token?.trim() || "",
  );
  const usernameValue = computed(() => username.value.trim());
  const timestampFromValue = computed(() =>
    parseTimestamp(timestampFrom.value),
  );
  ...
  const canCreateToken = computed(
    () => !createLoading.value && !formValidationError.value,
  );
  ...
  const handleCreateToken = async () => {
    ...
    try {
      ...
      if (!key || !secret) {
        throw new Error(
          "Token was created but key/secret is missing in response.",
        );
      }
      ...
      createToast.value = {
        ...
        title: "Token Created",
        message:
          "Save the generated credential now. Secret is not retrievable later.",
      };
    } catch (error: any) {
      createToast.value = {
        type: "error",
  ...
  const templatePermissionFingerprint = computed(() =>
    JSON.stringify({
